...
CODES_DIR = os.path.join(BASE_DIR, "testcases", "Sequential", "Codes")
GOLDEN_DIR = os.path.join(BASE_DIR, "testcases", "Sequential", "Outputs")
...
TESTBENCH_NAMES = ["seq_processor_tb.v", "seq_tb.v", "processor_tb.v"]

# Marks: Test 00-13 = 2 marks each, Test 14-20 = 6 marks each, total = 70
MARKS = {}
for i in range(14):
    MARKS[i] = 2
...
def main():
    ...
    with open(CSV_FILE, 'w', newline='') as f:
        writer = csv.writer(f)
        header = ['Team No', 'Roll No'] + [f'Test_{i:02d}' for i in range(21)] + ['Total']
    ...
    for team_no in sorted(teams.keys()):
        ...
        with open(CSV_FILE, 'a', newline='') as f:
            writer = csv.writer(f)
            m = marks if len(marks) == 21 else marks + [0] * (21 - len(marks))
            total_m = sum(m)
            if not roll_numbers:
